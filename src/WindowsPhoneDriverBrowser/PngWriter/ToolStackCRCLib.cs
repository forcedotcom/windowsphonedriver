...
namespace ToolStackCRCLib
{
    ...
    public class CRC32
    {
        ...
        private uint crcValue = (uint)0xffffffffL;
        ...
        public uint crc()
        {
            return crcValue ^ (uint)0xffffffffL;
        }
        ...
        public void addToCRC(byte[] buf, int len, uint offset)
        {
            ...
            for (n = offset; n < offset + len; n++)
            {
                c = crcTable[(c ^ buf[n]) & (uint)0xff] ^ (c >> 8);
            }
    ...
    public class Adler32
    {
        ...
        */
        ...
        /// <returns>An unsigned 32bit integer representing the current Adler32.</returns>
        [CLSCompliant(false)]
        public uint adler(byte[] data, int len)
        {
